...
async function handleFetch(event) {
  ...
  if (isDocumentGetRequest(event.request)) {
    ...
    return caches.open(DOCUMENT_CACHE).then((cache) => fetch(event.request.clone()).then((response) => {
      cache.put(event.request, response.clone());
      return response;
    }).catch(async (error) => {
      console.debug("Serving document from network failed, falling back to cache", url2.pathname);
      const response = await caches.match(event.request);
      if (!response) {
        throw error;
      }
